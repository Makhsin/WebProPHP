...
<head>
    ...
    $car = $_SESSION['car'];
    $index = array_search($car,$car_model);
    $photo = 'Pic/' . $car . '.png';
...
<body>
    ...
    <div class="row">
        ...
        <div class="columnRight">
            ...
            if (isset($_GET['sub'])) { //ถ้ากดปุ่มคำนวณ
                @$price = $_GET['price'];
                @$price1 = $_GET['price1'];
                $term_day = $_GET['day'];
                ...
                if (isset($price)) {
                    $check_float = 1;
                    $persen = $price * 100; //เอาจุดแปลงเป็นเปอร์เซนด์
                } else {
                    $price = $price1;
                    $persen = ($price / $carprice[$index]) * 100; //นำเงินดาวมาเป็นเปอร์เซ็น
                }
